...
const MapNavigationComponent = ({ props }) => {
  ...
  const mountMap = async (items, activeId) => {
    ...
    try {
      ...
      if (activePoint) mapInstance.panTo(activePoint, { animate: false });
      setStatus(
        locatedCount
  ...
  return {
    next(message = {}) {
      ...
      const types = [
        "Todos",
        ...new Set(source.flatMap(propertyTypeValues)),
      ];

      const filtered = source.filter((property) => {
        const propertyTypes = propertyTypeValues(property);
        return (
          (selectedType === "Todos" || propertyTypes.includes(selectedType)) &&
          (selectedCity === "Todos" || normalizedCity(property) === selectedCity) &&
